...
namespace _02._Judge_Nested_Dict
{
    class Program
    {
        static void Main(string[] args)
        {
            Dictionary<string, Dictionary<string, int>> contestDict = new Dictionary<string, Dictionary<string, int>>();
            ...
            while ((input = Console.ReadLine()) != "no more time")
            {
                ...
                else if (!contestDict[contest].ContainsKey(userName))
                {
                    contestDict[contest].Add(userName, points);
                }
                else if (contestDict[contest][userName] < points)
                {
            ...
            foreach (var contest in contestDict)
            {
                ...
                foreach (var user in contest.Value
                    ...
                    .ThenBy(x => x.Key))
                {
                    Console.WriteLine($"{positionInContest}. {user.Key} <::> {user.Value}");
                    positionInContest++;
                    userList.Add(new User(user.Key, user.Value));
                }
            }
