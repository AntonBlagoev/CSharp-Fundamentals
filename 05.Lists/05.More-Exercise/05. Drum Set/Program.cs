...
namespace _05._Drum_Set
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<int> drumSetInitialQualityList = Console.ReadLine().Split().Select(int.Parse).ToList();
            List<int> drumSetList = drumSetInitialQualityList.ToList();
            ...
            while ((input = Console.ReadLine()) != "Hit it again, Gabsy!")
            {
                ...
                {
                    int tmpValue = drumSetList[i] - hitPower;
                    drumSetList[i] = tmpValue;

                    if (drumSetList[i] <= 0)
                    {
                        int drumPrice = drumSetInitialQualityList[i] * 3;
                        if (savings >= drumPrice)
                        {
                            drumSetList[i] = drumSetInitialQualityList[i];
                            savings -= drumPrice;
                        }
                        else
                        {
                            drumSetList.RemoveAt(i);
                            drumSetInitialQualityList.RemoveAt(i);
                            i--;
                        }
                    }
                }
            }
            Console.WriteLine(string.Join(" ", drumSetList));
 ...
 * She has a drum set but the different drums have different origins – some she bought, some are gifts, so they are all of different quality. 
 ...
 * Until you receive the command "Hit it again, Gabsy!", you will be receiving an integer: 
 * the hit power Gabsy applies on each drum while practicing. When the power is applied, 
 * you should decrease the value of the drum's quality with the current power.
 * When a certain drum reaches 0 quality, it breaks. Then Gabsy should buy a replacement. 
 * She needs to buy the same model. Therefore, its quality will be the same as the initial quality of the broken drum. 
